...
public interface PaymentRequestRepository extends JpaRepository<PaymentRequest, Long> {
    ...
    @Query(value = "SELECT count(p) FROM PaymentRequest p WHERE p.status = 'Need payment'")
    int getPrWaitingForPayment();

    List<PaymentRequest> getAllByStatusEquals(String status);

    Optional<PaymentRequest> findByPaymentRequestId(Long id);
}
